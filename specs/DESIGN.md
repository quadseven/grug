...
| Term | Definition |
|---|---|
| **`grug-cave-jobs` / `grug-cave-results`** | The airlock — two SQS **FIFO** queues (Pulumi). `jobs`: webhook Lambda → connector. `results`: connector → webhook Lambda (consumed via SQS event-source mapping). FIFO gives free content-dedup + per-install ordering (`MessageGroupId=install_id`); **long polling** (`WaitTimeSeconds=20`) is mandatory (free-tier guard). DLQ + redrive + monitors are the resilience slice (#312). SQS is greenfield in grug here; ADR-0004's rerun queue (#305) reuses this scaffolding (extraction deferred, rule-of-three). |
