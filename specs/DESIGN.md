...
| **`Verdict` / `verdict()`** | The at-a-glance badge — `block` / `warn` / `pass` / `errored` — derived from a Check verdict's raw facts by the SINGLE pure function `review_types.verdict(conclusion, findings_count, degraded_reason)` (ADR-0003). Precedence: `degraded_reason` set → `errored` (never `pass` — "no lies"); `conclusion=="failure"` → `block`; `findings_count>0` → `warn`; else `pass`. Both the write path and the future `/activity` read path call this ONE function; the frontend renders it verbatim (single source of truth). |
