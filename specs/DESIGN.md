...
| **Webhook dispatch budget** | `receive_github_webhook` runs `dispatch()` SYNCHRONOUSLY before ACKing GitHub, and the Elder path makes 1–2 LLM calls. The webhook Lambda is `timeout_seconds=60` (#252, was 15) so the diff-fetch (10s) + review LLM (30s) + publish path completes gracefully rather than being killed mid-flight; no single per-request httpx timeout reaches the budget. GitHub's ~10s delivery timeout still fires on slow reviews (cosmetic — GitHub doesn't auto-redeliver). Proper fix: offload the Elder LLM dispatch off the ACK path (async self-invoke/SQS), tracked in #272. |
