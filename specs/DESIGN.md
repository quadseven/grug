...
| **`FallbackJob` / `FallbackResult`** | The cross-repo message contract (webhook-only dataclasses, NOT mirrored — like `async_dispatch`). `FallbackJob` carries install/repo/pr/head coords + the diff **inline** (small-diff scope; S3 spillover is #311) and **no GitHub credential**. `FallbackResult` carries the connector's findings + a `degraded` marker. The connector consumes the job and produces the result; it never calls GitHub. |
