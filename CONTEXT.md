...
| **Elder** (the **code-reviewer** persona) | The oldest grug, many winters of bad code behind him — Elder seen many a grug fall in the same pit. Elder read your markings one line at a time and name the bad omen before it bite: the null in the dark, the race by the river, the error swallowed in silence. = the LLM **code review** pass PLUS the deterministic **security suite** (`Grug — Code Review`), advisory by default. Four candidate sources run on every diff — SAST (`sast.py`, Semgrep + builtin), dependency-CVE (`sca.py`, OSV), committed secrets (`secret_scan.py`), IaC misconfig (`iac_scan.py`) — all graded by the LLM **exploitability judge** (`judge_candidates`) before anything reaches the PR; recall/precision tracked by the committed SAST benchmark (`sast_benchmark/`, `benchmark.sast.yml`). Code dir `personas/code_reviewer/` (key `code_reviewer`); canonical name **`elder`** (already `_PERSONA = "Elder"` in dispatch). Speaks in the wise-caveman voice (the `_VOICE` clause in `code_review_prompt.py`). |
...
A large set of modules (authoritative list: `scripts/check-mirrored-files.sh`, enforced per-PR by drift-lint) exists as byte-identical copies under both `services/api/` and `services/webhook/`. A representative sample: See [ADR-0001](docs/adr/0001-mirror-with-rule-of-three-deferral.md) for the load-bearing reasoning.
