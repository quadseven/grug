...
| **Elder** (the **code-reviewer** persona) | The oldest grug, many winters of bad code behind him — Elder seen many a grug fall in the same pit. Elder read your markings one line at a time and name the bad omen before it bite: the null in the dark, the race by the river, the error swallowed in silence. = the LLM **code review** pass (`Grug — Elder`; legacy alias `Grug — Code Review`). Apex path: **tiered Cave review** (ADR-0019 / #645), Swift Hunt settle, in_progress check on enqueue, Markings Board publish, Lore + Omen fused. Production review jobs enter the durable rerun FIFO, wait for a 90-second quiet snapshot, and cancel/re-enqueue when base, head, title, or body changes before publication. Default `GRUG_REVIEW_DEPTH=tiered` always runs the Cave **coder** arm; the Cave **reasoner** arm joins only on escalation (large diff, high-risk paths, explicit deep-review, or calibration sample). `deep` restores always-on dual-arm merge; `fast` is coder-first with reasoner only if coder fails. SaaS (OpenRouter review-only Opus / Poolside) is overload fallback only. Judge and write-authorized maintainer reaction labels fan out only to the producer spans that actually exist. Confirmed reactions refresh positive practices/examples; false positives become bounded AVOID guidance. Judge/Teller/`/grug ask` keep the lighter OpenRouter model. (The deterministic security suite that used to ride inside Elder is now **Guard**, #466/ADR-0012.) Code dir `personas/code_reviewer/` (key `code_reviewer`); canonical name **`elder`** (already `_PERSONA = "Elder"` in dispatch). Speaks in the wise-caveman voice (the `_VOICE` clause in `code_review_prompt.py`). |
...
| Term | Definition |
|---|---|
| **Definition of Ready (DoR) / Chief's plan** | The standard a PR description must meet before it can merge. Enforced as a set of `CheckResult`s combined into a single GitHub check-run named **`Grug — Chief`** (legacy alias `Grug — Definition of Ready` dual-posted during cutover). Process jargon "DoR" remains in code paths; product surfaces say Chief. |
