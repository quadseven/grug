# MIRRORED — sibling at services/webhook/personas/smasher/webhook_dispatch.py; keep in lockstep. See docs/adr/0001-mirror-with-rule-of-three-deferral.md.
...
`async_dispatch` is webhook-only, so the imports stay inside the function (this
file is mirrored into the api service where the loop never runs).
"""
