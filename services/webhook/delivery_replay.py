...
Idempotency (acceptance #4) without any persistence: GitHub deliveries share a
`guid` across an event's original delivery and its redeliveries, and a delivery
is SUCCESS when its `status_code` is 200-399. So we only redeliver guids where
NO delivery succeeded - the moment a redelivery returns 2xx, that guid has a
success and the next pass skips it. (The window is bounded, so even if GitHub
ever stopped sharing guids across redeliveries, the worst case is a redundant
redeliver, never a duplicate check: `post_check_run` is idempotent on
(name, head_sha).)
"""
...
def guids_needing_replay(deliveries: list[Delivery]) -> list[int]:
    ...
    for attempts in by_guid.values():
        if any(_is_success(a.status_code) for a in attempts):
            continue
        latest = max(attempts, key=lambda a: (a.delivered_at, a.id))
