...
Closes #45. Earlier `_SECTION_PAT = re.compile(r'^##+\\s+(.+?)\\s*$')`
matched 2-or-more `#`, so a `### Sub` inside `## Acceptance criteria`
became the section divider and the section text returned empty —
acceptance check failed on PRs with nested headings (PR #43 hit this).
"""
